...
class Seller:
    ...
    def get_improvement_data(self,months,scores,content='profit',Turn_on_percentage=False):
        ...
        total=seller[['cost_of_reviews','profits','IT_cost','n_orders']].sum()
        ...
        for m in months:
            ...
            for s in scores:
                #print((m,s))
                stra=stra0[(stra0['review_score']<s) | (stra0['profits']<=0)]
                # we want to show improvement so multiuply by -1
                stra1=stra[['cost_of_reviews','profits','IT_cost','n_orders']].sum().apply(lambda x: -x)
